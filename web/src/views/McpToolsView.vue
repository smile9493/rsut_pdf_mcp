        ...
        <div class="p-4 space-y-4">
          ...
          <div>
            <p class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-2">
              Wiki 工具
              <span class="ml-1 px-1 py-px rounded text-[9px] font-bold bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400">NEW</span>
            </p>
            <div class="space-y-1">
              <button
        ...
        <template v-if="selectedTool">
          <div class="flex-shrink-0 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/30 px-6 py-4">
            <div class="flex items-center gap-2 mb-2">
              <component :is="selectedTool.icon" class="w-4 h-4" :class="selectedTool.category === 'wiki' ? 'text-emerald-600 dark:text-emerald-400' : 'text-blue-600 dark:text-blue-400'" />
          ...
          <div class="flex-shrink-0 px-6 py-4 border-b border-slate-100 dark:border-slate-700/50 space-y-3">
            ...
            <div class="flex items-center gap-3 pt-1">
              <button
                ...
                :class="selectedTool?.category === 'wiki' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-blue-600 hover:bg-blue-700'"
                @click="executeTool"
              >
...
import { DocumentTextIcon, CubeIcon, DocumentIcon, MagnifyingGlassIcon, ServerIcon, CloudIcon } from '@heroicons/vue/24/outline'
...
const toolNameMap: Record<string, string> = {
  extract_text: '文本提取', extract_structured: '结构化提取', get_page_count: '页数查询',
  search_keywords: '关键词搜索', extrude_to_server_wiki: '服务端Wiki构建', extrude_to_agent_payload: '本地Wiki投影'
}
...
const canExecute = computed(() => {
  if (!filePath.value || !selectedTool.value) return false
...
const checkMcpStatus = () => {
  ...
  if (s) { try { mcpReady.value = !!JSON.parse(s).serverCommand } catch { mcpReady.value = false } }
  else mcpReady.value = false
}
...
const executeTool = async () => {
  ...
  try {
    ...
    if (selectedTool.value!.name === 'search_keywords') { args.keywords = keywords.value.split(',').map(k => k.trim()); args.case_sensitive = caseSensitive.value }
    if (selectedTool.value!.name === 'extract_structured') { args.enable_highlight = enableHighlight.value }
    if (selectedTool.value!.name === 'extrude_to_server_wiki') { args.wiki_base_path = wikiBasePath.value }
    ...
    let parsed: any
    try { parsed = selectedTool.value!.name === 'search_keywords' ? JSON.parse(text) : text } catch { parsed = text }
